...
//            let porcent = partialPorcent * 100
    ...
    func someTransaction(type: TransactionType) -> Double {
        let filterExpense = transactions.filter( { $0.type == type })
        
        let result = filterExpense.reduce(0) { acumulador, transaction in
            acumulador + transaction.amount
        }
